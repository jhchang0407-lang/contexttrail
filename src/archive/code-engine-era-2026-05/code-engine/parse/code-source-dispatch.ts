...
export function extractCodeIndexArtifactsFor(args: ExtractDispatchArgs): CodeIndexArtifacts {
  ...
  switch (lang) {
    case "typescript":
    case "javascript":
      return withRoleFactsArtifacts(
        extractCodeIndexArtifacts({
...
function withRoleFactsArtifacts(artifacts: CodeIndexArtifacts): CodeIndexArtifacts {
  const facts = withCodeRoleFacts(artifacts.facts);
  return {
    facts,
...
export function extractCodeSourceFactsFor(args: ExtractDispatchArgs): CodeSourceFacts {
  ...
  switch (lang) {
    case "typescript":
    case "javascript":
      return withCodeRoleFacts(extractCodeSourceFacts({
        source_path: args.source_path,
        content: args.content,
        corpus_root: args.corpus_root,
      }));
    case "python":
      return withCodeRoleFacts(extractPythonCodeSourceFacts({
        source_path: args.source_path,
        content: args.content,
        corpus_root: args.corpus_root,
      }));
    case "go":
      return withCodeRoleFacts(extractGoCodeSourceFacts({
        source_path: args.source_path,
        content: args.content,
        ...(args.go_module_prefix ? { module_prefix: args.go_module_prefix } : {}),
      }));
    case "rust":
      return withCodeRoleFacts(extractRustCodeSourceFacts({
        source_path: args.source_path,
        content: args.content,
      }));
    default:
      return withCodeRoleFacts({
