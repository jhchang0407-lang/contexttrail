...
export function extractCodeIndexArtifacts(
  ...
  try {
    ...
    imports = collectGraphImports(sf, args.source_path);
...
function collectGraphImports(sf: ts.SourceFile, sourcePath: string): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const stmt of sf.statements) {
    ...
    const resolved = raw.startsWith(".")
      ? resolveRelative(sourcePath, raw)
      : graphPackageImport(raw);
...
function graphPackageImport(spec: string): string | null {
  if (!isGraphPackageImport(spec)) return null;
  return spec;
}

function isGraphPackageImport(spec: string): boolean {
  if (!spec || spec.startsWith("node:")) return false;
  if (spec.startsWith("data:") || spec.startsWith("http:") || spec.startsWith("https:")) {
    return false;
  }
  if (TS_BUILTIN_MODULES.has(spec)) return false;
...
const TS_BUILTIN_MODULES = new Set([
  "assert",
  "buffer",
  "child_process",
  "cluster",
