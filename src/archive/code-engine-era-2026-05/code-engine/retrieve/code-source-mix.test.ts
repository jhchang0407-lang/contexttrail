...
afterEach(() => {
  ...
  delete process.env.RETRIEVAL_CODE_LANE_PACKAGE_DEPENDENCY_FANOUT_PROMOTED;
  delete process.env.RETRIEVAL_CODE_LANE_ROLE_FAMILY_FANOUT_PROMOTED;
  delete process.env.RETRIEVAL_CODE_LANE_SYMBOL_REFERENCE_FANOUT_PROMOTED;
  delete process.env.RETRIEVAL_CODE_LANE_COCHANGE_FANOUT_PROMOTED;
  delete process.env.RETRIEVAL_CODE_LANE_TAIL_RECALL_RESERVE_FRACTION;
});
...
describe("buildCodeRankedEntries", () => {
  ...
  it("returns code entries with kind='code' and a code-shaped contexttrail", () => {
    ...
    expect(top.retrieval_confidence.level).toBe("high");
    expect(top.retrieval_confidence.retry_recommended).toBe(false);
    expect(top.retrieval_confidence.reasons).toContain("exact_symbol_anchor");
  });
  ...
  it("reports repo-family path/facts shadow candidates without changing ranked output", () => {
    ...
    expect(target?.confidence.level).not.toBe("low");
    expect(target?.confidence.retry_recommended).toBe(false);
  });
  ...
  it("adds workspace-family support files through role metadata fanout", () => {
    seedCodeFile({
      path: "packages/preact-query-devtools/src/PreactQueryDevtools.tsx",
      imports: [],
      purpose: "Floating dock panel owner.",
      symbols: [{ name: "PreactQueryDevtools", kind: "function" }],
      signatures: ["export function PreactQueryDevtools(): JSX.Element"],
      chunks: [{
        stable_key: "packages/preact-query-devtools/src/PreactQueryDevtools.tsx::PreactQueryDevtools",
        symbol_path: "PreactQueryDevtools",
        code_role: "declaration",
        declaration_kind: "function",
        exported: true,
        body: "export function PreactQueryDevtools() { return floatingDockPanel(); }",
        start_line: 1,
        end_line: 1,
      }],
    });
    seedCodeFile({
      path: "packages/solid-query-devtools/src/index.ts",
      imports: [],
      purpose: "Solid package public exports.",
      symbols: [],
      signatures: [],
      chunks: [{
        stable_key: "packages/solid-query-devtools/src/index.ts::orientation",
        symbol_path: null,
        code_role: "orientation",
        declaration_kind: null,
        exported: false,
        body: "Code file: packages/solid-query-devtools/src/index.ts\nexport * from './SolidQueryDevtools';",
        start_line: 1,
        end_line: 1,
      }],
    });

    const query = "floating dock panel owner";
    const baseline = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    expect(baseline.map((entry) => entry.source_path)).not.toContain(
      "packages/solid-query-devtools/src/index.ts",
    );

    process.env.RETRIEVAL_CODE_LANE_OWNER_FANOUT_PROMOTED = "on";
    process.env.RETRIEVAL_CODE_LANE_ROLE_FAMILY_FANOUT_PROMOTED = "on";
    const promoted = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    const config = promoted.find((entry) =>
      entry.source_path === "packages/solid-query-devtools/src/index.ts"
    );

    expect(config?.import_traversed).toBe(true);
    expect(config?.support_cluster?.reason).toBe("role_family_fanout");
    expect(config?.support_cluster?.seed_source_path).toBe(
      "packages/preact-query-devtools/src/PreactQueryDevtools.tsx",
    );
  });

  it("adds package dependency fanout from manifest-backed workspace edges", () => {
    seedCodeFile({
      path: "packages/angular-query-experimental/src/useQuery.ts",
      imports: [],
      purpose: "Angular query adapter owner.",
      symbols: [{ name: "injectQuery", kind: "function" }],
      signatures: ["export function injectQuery(): QueryResult"],
      packageFacts: packageFacts({
        root: "packages/angular-query-experimental",
        name: "@tanstack/angular-query-experimental",
        dependencies: ["@tanstack/query-core"],
      }),
      chunks: [{
        stable_key: "packages/angular-query-experimental/src/useQuery.ts::injectQuery",
        symbol_path: "injectQuery",
        code_role: "declaration",
        declaration_kind: "function",
        exported: true,
        body: "export function injectQuery() { /* hardcoded adapter keys */ return hardcodedAdapterKeys(); }",
        start_line: 1,
        end_line: 1,
      }],
    });
    seedCodeFile({
      ...
      imports: [],
      purpose: "Core query public surface.",
      symbols: [],
      signatures: [],
      packageFacts: packageFacts({
        root: "packages/query-core",
        name: "@tanstack/query-core",
        dependents: ["@tanstack/angular-query-experimental"],
      }),
      chunks: [{
        stable_key: "packages/query-core/src/index.ts::orientation",
        symbol_path: null,
        code_role: "orientation",
        declaration_kind: null,
        exported: false,
        body: "Code file: packages/query-core/src/index.ts\nexport * from './queryClient';",
        start_line: 1,
        end_line: 1,
      }],
    });

    const query = "hardcoded adapter keys";
    const baseline = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    process.env.RETRIEVAL_CODE_LANE_OWNER_FANOUT_PROMOTED = "on";
    process.env.RETRIEVAL_CODE_LANE_PACKAGE_DEPENDENCY_FANOUT_PROMOTED = "on";
    const promoted = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    const dependency = promoted.find((entry) =>
      entry.source_path === "packages/query-core/src/index.ts"
    );

    expect(dependency?.import_traversed).toBe(true);
    expect(dependency?.support_cluster?.reason).toBe(
      "package_dependency_fanout",
    );
    expect(dependency?.support_cluster?.seed_source_path).toBe(
      "packages/angular-query-experimental/src/useQuery.ts",
    );
  });

  it("adds symbol-reference fanout from owner references to exported helpers", () => {
    seedCodeFile({
      path: "packages/runtime/src/owner.ts",
      imports: [],
      purpose: "Runtime owner for refresh work.",
      symbols: [{ name: "refreshRuntimePipeline", kind: "function" }],
      signatures: ["export function refreshRuntimePipeline(): void"],
      chunks: [{
        stable_key: "packages/runtime/src/owner.ts::refreshRuntimePipeline",
        symbol_path: "refreshRuntimePipeline",
        code_role: "declaration",
        declaration_kind: "function",
        exported: true,
        body: "export function refreshRuntimePipeline() { return createMigrationPlan(); }",
        start_line: 1,
        end_line: 1,
      }],
    });
    seedCodeFile({
      path: "packages/runtime/src/planner/create-plan.ts",
      imports: [],
      purpose: "Planner implementation.",
      symbols: [{ name: "createMigrationPlan", kind: "function" }],
      signatures: ["export function createMigrationPlan(): Plan"],
      chunks: [{
        stable_key: "packages/runtime/src/planner/create-plan.ts::createMigrationPlan",
        symbol_path: "createMigrationPlan",
        code_role: "declaration",
        declaration_kind: "function",
        exported: true,
        body: "export function createMigrationPlan() { return {}; }",
        start_line: 1,
        end_line: 1,
      }],
    });

    const query = "refresh pipeline owner";
    const baseline = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    expect(baseline.map((entry) => entry.source_path)).not.toContain(
      "packages/runtime/src/planner/create-plan.ts",
    );

    process.env.RETRIEVAL_CODE_LANE_OWNER_FANOUT_PROMOTED = "on";
    process.env.RETRIEVAL_CODE_LANE_SYMBOL_REFERENCE_FANOUT_PROMOTED = "on";
    const promoted = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    const helper = promoted.find((entry) =>
      entry.source_path === "packages/runtime/src/planner/create-plan.ts"
    );

    expect(helper?.import_traversed).toBe(true);
    expect(helper?.support_cluster?.reason).toBe("symbol_reference_fanout");
    expect(helper?.support_cluster?.seed_source_path).toBe(
      "packages/runtime/src/owner.ts",
    );
  });

  it("adds co-change fanout from historical patch-set evidence", () => {
    seedCodeFile({
      path: "packages/client/src/runtime/owner.ts",
      imports: [],
      purpose: "Runtime query insight owner.",
      symbols: [{ name: "applyQueryInsight", kind: "function" }],
      signatures: ["export function applyQueryInsight(): void"],
      cochangeFacts: {
        related_paths: [
          {
            source_path: "packages/adapter-d1/helpers/build.ts",
            count: 3,
          },
        ],
      },
      chunks: [{
        stable_key: "packages/client/src/runtime/owner.ts::applyQueryInsight",
        symbol_path: "applyQueryInsight",
        code_role: "declaration",
        declaration_kind: "function",
        exported: true,
        body: "export function applyQueryInsight() { return queryInsight(); }",
        start_line: 1,
        end_line: 1,
      }],
    });
    seedCodeFile({
      path: "packages/adapter-d1/helpers/build.ts",
      imports: [],
      purpose: "Adapter build helper.",
      symbols: [],
      signatures: [],
      chunks: [{
        stable_key: "packages/adapter-d1/helpers/build.ts::orientation",
        symbol_path: null,
        code_role: "orientation",
        declaration_kind: null,
        exported: false,
        body: "Code file: packages/adapter-d1/helpers/build.ts\nBuilds the adapter helper package.",
        start_line: 1,
        end_line: 2,
      }],
    });

    const query = "query insight owner";
    const baseline = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    expect(baseline.map((entry) => entry.source_path)).not.toContain(
      "packages/adapter-d1/helpers/build.ts",
    );

    process.env.RETRIEVAL_CODE_LANE_OWNER_FANOUT_PROMOTED = "on";
    process.env.RETRIEVAL_CODE_LANE_COCHANGE_FANOUT_PROMOTED = "on";
    const promoted = buildCodeRankedEntries({
      db,
      query,
      enabled: true,
    ...
    const helper = promoted.find((entry) =>
      entry.source_path === "packages/adapter-d1/helpers/build.ts"
    );

    expect(helper?.import_traversed).toBe(true);
    expect(helper?.support_cluster?.reason).toBe("cochange_fanout");
    expect(helper?.support_cluster?.seed_source_path).toBe(
      "packages/client/src/runtime/owner.ts",
    );
  });
...
function seedCodeFile(args: {
  ...
  cochangeFacts?: {
    related_paths: Array<{ source_path: string; count: number }>;
  };
  chunks: Array<{
    stable_key: string;
  ...
  upsertCodeSource(db, {
    facts: {
      ...
      ...(args.packageFacts ? { package_facts: args.packageFacts } : {}),
      ...(args.cochangeFacts ? { cochange_facts: args.cochangeFacts } : {}),
    },
...
function packageFacts(args: {
  ...
  return {
    ...
    manifest_path: `${args.root}/package.json`,
    internal_dependency_names: args.dependencies ?? [],
    internal_dependency_roots: (args.dependencies ?? []).map(packageRootForName),
    internal_dependent_names: args.dependents ?? [],
    internal_dependent_roots: (args.dependents ?? []).map(packageRootForName),
    script_names: [],
    export_keys: ["."],
  };
}

function packageRootForName(name: string): string {
