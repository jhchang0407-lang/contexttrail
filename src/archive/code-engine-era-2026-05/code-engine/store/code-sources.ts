...
import { withCodeRoleFacts } from "../facts/code-role-facts.js";
...
INSERT INTO code_sources (
  ...
  file_purpose, imports, role_facts, package_facts, cochange_facts, indexed_at
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(source_path) DO UPDATE SET
  ...
  role_facts=excluded.role_facts,
  package_facts=excluded.package_facts,
  cochange_facts=excluded.cochange_facts,
...
export function upsertCodeSource(
  ...
  const facts = withCodeRoleFacts(args.facts);
  const tx = db.transaction(() => {
    db.prepare(UPSERT_SQL).run(
      ...
      JSON.stringify(facts.role_facts),
      facts.package_facts ? JSON.stringify(facts.package_facts) : null,
      facts.cochange_facts ? JSON.stringify(facts.cochange_facts) : null,
      args.indexed_at,
    );
...
type CodeSourceRow = {
  ...
  imports: string;
  role_facts?: string | null;
  package_facts?: string | null;
  cochange_facts?: string | null;
  indexed_at: string;
};

function rowToStored(row: CodeSourceRow): StoredCodeSource {
  const roleFacts = parseRoleFacts(row.role_facts);
  const packageFacts = parsePackageFacts(row.package_facts);
  const cochangeFacts = parseCochangeFacts(row.cochange_facts);
  return {
    facts: withCodeRoleFacts({
      ...
      ...(roleFacts ? { role_facts: roleFacts } : {}),
      ...(packageFacts ? { package_facts: packageFacts } : {}),
      ...(cochangeFacts ? { cochange_facts: cochangeFacts } : {}),
    }),
...
function parseRoleFacts(raw: string | null | undefined): CodeSourceRoleFacts | null {
  return parseJsonOrNull<CodeSourceRoleFacts>(raw);
}

function parsePackageFacts(
  raw: string | null | undefined,
): CodeSourcePackageFacts | null {
  return parseJsonOrNull<CodeSourcePackageFacts>(raw);
}

function parseCochangeFacts(
  raw: string | null | undefined,
): CodeSourceCochangeFacts | null {
  return parseJsonOrNull<CodeSourceCochangeFacts>(raw);
}
