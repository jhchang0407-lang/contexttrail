...
describe("agent-completion probe — assembly gate wiring (PRD-0029 / 29.2)", () => {
  ...
  it("summarizes target-file autopsy buckets for future method diagnosis", () => {
    const rows: AgentCompletionDetailedRow[] = [
      {
        ticket: "THO-autopsy",
        commit: "abc1234",
        changedFiles: ["src/owner.ts", "src/support.ts", "src/missing.ts"],
        mentionedFiles: ["src/owner.ts"],
        srcOverlap: 1,
        srcTotal: 3,
        docOverlap: 0,
            ...
            targetFileAutopsy: [
              {
                ...
                topOwnerFile: "src/owner.ts",
                ownerRelations: ["same_file"],
                ownerCandidates: [{
                  sourcePath: "src/owner.ts",
                  rank: 1,
                  candidateRank: 1,
                  packRank: 1,
                  relations: ["same_file"],
                ...
                queryTokenCount: 3,
                pathTokenOverlap: 1,
                symbolTokenOverlap: 1,
                purposeTokenOverlap: 1,
                factTokenOverlap: 2,
                outcome: "pack_top3_hit",
              },
              {
                query: "owner support flow",
                targetFile: "src/support.ts",
                indexed: true,
                hasChunks: true,
                packRank: null,
                candidateRank: 42,
                ...
                candidateAdmitted: true,
                candidateShadow: false,
                evidenceFamilies: ["repo_family"],
                evidenceReasons: ["repo_map"],
                topOwnerFile: "src/owner.ts",
                ownerRelations: ["same_directory"],
                ownerCandidates: [{
                  sourcePath: "src/owner.ts",
                  rank: 1,
                  candidateRank: 1,
                  packRank: 1,
                  relations: ["same_directory"],
                }],
                queryTokenCount: 3,
                pathTokenOverlap: 1,
                ...
                factTokenOverlap: 1,
                outcome: "candidate_top100_hit",
              },
              {
                query: "owner support flow",
                targetFile: "src/missing.ts",
                indexed: true,
                hasChunks: true,
                packRank: null,
                candidateRank: null,
    ...
    expect(summary.targetFileAutopsySummary).toMatchObject({
      observations: 3,
      indexed: 3,
      withChunks: 3,
      packTopThreeHits: 1,
      packRankedHits: 1,
      candidateTopHundredHits: 2,
      queryObvious: {
        path: 2,
        symbol: 1,
        purpose: 1,
    ...
    expect(summary.targetFileAutopsySummary?.outcomes).toEqual([
      { outcome: "candidate_top100_hit", count: 1 },
      { outcome: "never_generated", count: 1 },
      { outcome: "pack_top3_hit", count: 1 },
    ]);
    expect(summary.candidateNoiseAutopsySummary).toMatchObject({
      observations: 1,
      admitted: 1,
      shadow: 0,
      packRanked: 1,
    ...
    expect(renderAgentCompletionReport(summary)).toContain("Target-file autopsy:");
    expect(renderAgentCompletionReport(summary)).toContain("never_generated=1");
    expect(renderAgentCompletionReport(summary)).toContain("Candidate-noise autopsy:");
    expect(renderAgentCompletionReport(summary)).toContain(
      "autopsy src/missing.ts: outcome=never_generated",
    );
  });
});
