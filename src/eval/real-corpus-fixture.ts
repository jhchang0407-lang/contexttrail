 ...
 * expected_evidence_covers_locked) are dropped. The seed measures
 * retrieval over docs only.
 *
...
function rankOfFirstMatch(
	...
	for (const entry of ranked) {
		...
		position += 1;
		if (acceptable.includes(sourceFromContextTrail(entry.contexttrail))) {
			return position;
		}
	}
	return 0;
...
export function classifyRealCorpusOutcome(
	...
	const rank = rankOfFirstMatch(input.ranked, acceptable);
	const top1Hit = rank === 1;
	const top3Hit = rank >= 1 && rank <= 3;
	const reciprocalRank = rank === 0 ? 0 : 1 / rank;

	let failureClass: RealCorpusFailureClass = "none";
	if (!top3Hit) failureClass = "answer_recall_miss";
	else if (!top1Hit) failureClass = "answer_ordering_miss";
	else if (input.actual_query_mode !== input.expected_query_mode)
		failureClass = "query_mode_miss";
	else if (
		...
		!input.mustIncludeSources.every((source) =>
			input.ranked.some(
				(r) =>
					r.kind === "chunk" && sourceFromContextTrail(r.contexttrail) === source,
			),
		)
	) {
		...
		for (const entry of cases) {
			...
			const top1Acceptable = isSignalEmptyCase
				...
				: top3FirstChunk !== undefined &&
					acceptableTopSources.includes(
						sourceFromContextTrail(top3FirstChunk.contexttrail),
					);
			...
			const chunkCorrect = top1Acceptable
